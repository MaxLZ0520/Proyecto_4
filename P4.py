...
SNR = 100   # relación señal-a-ruido del canal
...
#Se modifica la funcion modulador para dos portadores 
def modulador(bits, fc, mpp):
    '''Un método que simula el esquema de
    ...
    :return: Un vector con la señal modulada
    :return: Un valor con la potencia promedio [W]
    :return: La onda portadora con coseno
    :return: La onda portadora con seno 
    '''
    # 1. Parámetros de la 'señal' de información (bits)
    N = len(bits)  # Cantidad de bits
    ...
    #Este for nos ayuda a recorrer N donde se encuentran todos los bits, pero tomando en cuenta el metodo 16QAM,
    #por lo tanto toma en cuenta cada bit y la ubicacion de los mismos en el mapa de la constelacion
...
def demodulador(senal_Rx, portadoraI, portadoraQ, mpp):
    ...
    # Demodulación
    for i in range(N):
        #Se limita la cantidad de informacion recuperada, hasta cierto punto
        if j+4 > N:  
            break   

        # Producto interno de dos funciones
        producto1 = senal_Rx[i*mpp: (i+1)*mpp] * portadoraI
        producto2 = senal_Rx[i*mpp: (i+1)*mpp] * portadoraQ
...
fc = 5000  # frecuencia de la portadora
mpp = 20   # muestras por periodo de la portadora
SNR = 200   # relación señal-a-ruido del canal
...
senal_Tx, Pm, portadoraI, portadoraQ = modulador(bits_Tx, fc, mpp)
...
# La señal demodulada
ax4.plot(senal_demodulada[0:600], color='b', lw=2) 
ax4.set_ylabel('$b^{\prime}(t)$')
ax4.set_xlabel('$t$ / milisegundos')
fig.tight_layout()
plt.show()
...
# Inicialización del proceso aleatorio con N realizaciones
A = [-3,-1,1,3]                # Se tiene 4 posibles valores para la amplitud
N = 16                         # Hay 16 posibilidades de la onda
H_t = np.empty((N, len(t)))    # N funciones del tiempo con T puntos
J_t = np.empty((N, len(t)))

# Para la señal se aplica cada valor de amplitud (-3, -1, 1, 3)
for i in A:
    ...
    L = -i*np.cos(2*np.pi*fc*t) - i*np.sin(2*np.pi*fc*t)
...
Rea, = plt.plot(t, P, lw=3, color = 'blue', label='Promedio realizaciones')

# Graficar el resultado teórico del valor esperado

E = np.mean(senal_Tx)*t
Teo, = plt.plot(t, E, '-.', lw=3, color = 'seagreen', label='Valor teorico')

# Mostrar las realizaciones, y su promedio calculado y teórico
plt.title('Realizaciones del proceso aleatorio')
plt.xlabel('$t$')
plt.ylabel('$x_i(t)$')
plt.legend([Rea, Teo], ['Promedio realizaciones', 'Valor teórico'])
plt.show()   
...
# Ahora se procede a obtener la Densidad espectral de potencia
...
plt.xlim(0,20000)
plt.grid()
plt.title('Densidad espectral de la potencia')
plt.ylabel(r'$S{xx}(\omega)$')
